...
*/
{
    ...
    *ktype =  UNKNOWN;
	...
	temp1[21]='\0';
	sprintf(ctx->errmes,"card %s is > 80.",card);
    ...
    if( i < 8 && i > 0) {
        sprintf(ctx->errmes,"Keyword #%d: Name %s is not left justified.",
           kpos,kname);
	wrterr(ctx,out,ctx->errmes,1);
    }
	...
	*p = '\0';
    ...
    if( !strcmp(kname,"END") ){
        *ktype =  COM_KEY;
        ...
        for( p = &card[8]; *p != '\0'; p++) {
            if(*p != '\x20' ){
		wrterr(ctx,out,"END keyword contains non-blank characters.",1);
		return 1;
            }
        }
	kname[3] = '\0';
               ...
               )
{
  ...
  "Keyword #%d, %s: The closing \"\'\" is missing in the string." ,
	...
	sprintf(ctx->errmes,"Keyword #%d, %s: Bad logical value \"%s\".",
         kpos,kname,kval);
	 wrterr(ctx,out,ctx->errmes,1);
    }
    if(errnum & BAD_NUM) {
	sprintf(ctx->errmes,"Keyword #%d, %s: Bad numerical value \"%s\".",
         kpos,kname,kval);
	wrterr(ctx,out,ctx->errmes,1);
    }
    ...
    int check_str(fv_context *ctx, FitsKey* pkey, FILE *out)
{
    if(pkey->ktype == UNKNOWN && *(pkey->kvalue) == 0) {
        sprintf(ctx->errmes,"Keyword #%d, %s has a null value; expected a string.",
        pkey->kindex,pkey->kname);
        wrterr(ctx,out,ctx->errmes,1);
        return 0;
    } else if(pkey->ktype != STR_KEY) {
        sprintf(ctx->errmes,"Keyword #%d, %s: \"%s\" is not a string.",
...
{
    if(pkey->ktype == UNKNOWN && *(pkey->kvalue) == 0) {
        sprintf(ctx->errmes,"Keyword #%d, %s has a null value; expected an integer.",
        pkey->kindex,pkey->kname);
        wrterr(ctx,out,ctx->errmes,1);
        return 0;
    } else if(pkey->ktype != INT_KEY) {
        sprintf(ctx->errmes,"Keyword #%d, %s: value = %s is not an integer.",
        pkey->kindex,pkey->kname, pkey->kvalue);
...
{
    if(pkey->ktype == UNKNOWN && *(pkey->kvalue) == 0) {
        sprintf(ctx->errmes,"Keyword #%d, %s has a null value; expected a float.",
        pkey->kindex,pkey->kname);
        wrterr(ctx,out,ctx->errmes,1);
        return 0;
